...
export async function startServer({ port = 7456, returnServer = false } = {}) {
  ...
  const startChatRun = async (chatBody, run) => {
    ...
    try {
      ...
      run.child = child;
      if (
        (def.promptViaStdin || needsFilePrompt) &&
        child.stdin &&
  ...
  app.post("/api/proxy/anthropic/stream", async (req, res) => {
    ...
    if (!upstream.ok) {
      ...
      console.error(
        `[proxy:anthropic] upstream ${upstream.status}: ${safeErr.slice(0, 200)}`,
      );
      send(
        "error",
        createSseErrorPayload(
          "UPSTREAM_UNAVAILABLE",
          `upstream ${upstream.status}: ${safeErr}`,
      ...
      while ((match = buf.match(/\r?\n\r?\n/)) && match.index != null) {
        ...
        try {
          ...
          if (chunk.type === "content_block_delta") {
            ...
            console.warn(
              `[proxy:anthropic] skipped upstream event type=${chunk.type || "unknown"}`,
            );
          }
        } catch {
          if (process.env.NODE_ENV === "development") {
            console.warn(
              "[proxy:anthropic] skipped malformed upstream SSE frame",
            );
          }
        }
      }
  ...
  app.post("/api/proxy/stream", async (req, res) => {
    ...
    console.log(
      `[proxy] ${req.method} ${validated.parsed.hostname} model=${model} miMo=${isMiMo}`,
    );
    ...
    if (!upstream.ok) {
      ...
      const safeErr = redactAuthTokens(errText.slice(0, 500));
      console.error(
        `[proxy] upstream ${upstream.status}: ${safeErr.slice(0, 200)}`,
      );
      send(
        "error",
        createSseErrorPayload(
